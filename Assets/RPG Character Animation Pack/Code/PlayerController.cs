...
public class PlayerController : MonoBehaviour{
	...
	void Update(){
		...
		if(inputUnarmed && canAction && isGrounded && !isBlocking && weapon != WeaponType.UNARMED){
		...
		//Strafing
		if(inputStrafe || inputTargetBlock > 0.1f && canAction && weapon != WeaponType.RIFLE){  
			if(!isRelax){
				isStrafing = true;
		...
		if(Input.GetMouseButtonDown(0)){
			...
			if((weapon == WeaponType.TWOHANDBOW || weapon ==  WeaponType.TWOHANDCROSSBOW) && isAiming){
				animator.SetTrigger("Attack1Trigger");
			}
		}
	...
	public void Attack(int attackSide){
		if(canAction && isGrounded){
			...
			if(inputVec.magnitude == 0f){
				//Armed or Unarmed
				if(weapon == WeaponType.UNARMED || weapon == WeaponType.ARMED || weapon == WeaponType.ARMEDSHIELD){
					int maxAttacks = 3;
					int attackNumber = 0;
					//left attacks
					if(attackSide == 1){
				...
				else if(weapon == WeaponType.TWOHANDCLUB){
					int maxAttacks = 10;
					int attackNumber = Random.Range(1, maxAttacks);
					if(isGrounded){
						animator.SetTrigger("Attack" + (attackNumber).ToString() + "Trigger");
				...
				else if(weapon == WeaponType.TWOHANDSWORD){
					int maxAttacks = 11;
					int attackNumber = Random.Range(1, maxAttacks);
					if(isGrounded){
						animator.SetTrigger("Attack" + (attackNumber).ToString() + "Trigger");
	...
	public void Blocking(){
        ...
        if (inputTargetBlock < -0.1f && canAction && isGrounded)
        {
            if (!isBlocking){
	...
	public IEnumerator _SwitchWeapon(int weaponNumber){	
		...
		else if(weapon == WeaponType.STAFF || weapon == WeaponType.TWOHANDAXE || weapon == WeaponType.TWOHANDBOW || weapon == WeaponType.TWOHANDCROSSBOW || weapon == WeaponType.TWOHANDSPEAR || weapon == WeaponType.TWOHANDSWORD || weapon == WeaponType.RIFLE || weapon == WeaponType.TWOHANDCLUB){
		...
		else if(weapon == WeaponType.ARMED || weapon == WeaponType.SHIELD || weapon == WeaponType.ARMEDSHIELD){
	...
	public IEnumerator _UnSheathWeapon(int weaponNumber){
		...
		if(leftWeapon == 7){
			if(rightWeapon == 0){
				...
				animator.SetBool("Shield", true);
				weapon = WeaponType.ARMEDSHIELD;
			}
			animator.SetBool("Shield", true);
		}
		//Set weapon and animator to Armed
		if((leftWeapon > 6 || rightWeapon > 6) && weapon != WeaponType.RIFLE && weapon != WeaponType.TWOHANDCLUB){
