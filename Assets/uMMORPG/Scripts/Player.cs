...
public partial class Player : Entity
{
    ...
    string UpdateServer_DEAD()
    {
        ...
        if (EventRespawn())
        {
            ...
            Warp(start.position); // recommended over transform.position
            Revive(0.5f);
            return "IDLE";
        }
        if (EventMoveStart())
        ...
        if (state != "CASTING" || ignoreState)
        {
            ...
            if (CastCheckSelf(skill) && CastCheckTarget(skill))
            {
                ...
                else
                {
                    ...
                    // (use collider point(s) to also work with big entities)
                    Navigate(destination, skill.castRange * attackToMoveRangeRatio);
    ...
    public void CmdNpcBuyItem(int index, int amount)
    {
        ...
        {
            // valid amount?
            Item npcItem = new Item(npc.saleItems[index]);
    ...
    bool IsTradeOfferStillValid()
    {
        ...
        // all offered items are -1 or valid and tradeable?
        // (avoid Linq because it is HEAVY(!) on GC and performance)
    ...
    public void CmdNpcReviveSummonable(int index)
    {
            ...
            0 <= index && index < inventory.Count)
        {
            ...
            if (slot.amount > 0 && slot.item.data is SummonableItem summonable)
            {
                ...
                {
                    // enough gold?
                    if (gold >= summonable.revivePrice)
                    {
                        // pay for it, revive it
                        gold -= summonable.revivePrice;
                        slot.item.summonedHealth = summonable.summonPrefab.healthMax;
                        inventory[index] = slot;
                    }
                }
            }
            ...
            if (cast)
            {
                ...
                if (entity)
                {
                    ...
                    if (entity == target && entity != this && entity != activePet)
                    {
                        ...
                        {
                            // use collider point(s) to also work with big entities
                            Navigate(Utils.ClosestPoint(entity, transform.position), interactionRange);
                        }
