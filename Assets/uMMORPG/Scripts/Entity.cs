...
#pragma warning restore CS0649 // Field is never assigned to
    ...
    // navigate function with stopping distance to avoid redundant code.
    // (all entities use the same one, but it's still virtual just in case)
    ...
    //       entities will only be disabled but not destroyed. let's not worry
    //       about that for now.
    [Server]
    public void Recover()
    {
    ...
    [Server]
    public void CancelCastSkill(bool resetCurrentSkill = true)
    {
